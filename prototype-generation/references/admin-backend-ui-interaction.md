...
- 令牌管理页：全宽标准列表，字段为用户名、客户端、令牌、过期时间、状态，行内操作为下线，下线必须弹窗填写原因并记录操作留痕。
- 参数管理页：全宽标准列表，覆盖系统规则和业务规则，字段为名称、键、值、状态、系统标识，行内操作为修改，修改必须记录原因和审计日志。
- 敏感词管理页：全宽标准列表，维护敏感词/表达、适用范围、处理策略、状态，支持新增、修改、启停、导入导出或批量维护。
