...
- 信息推送页：不得使用左右分栏；使用全宽内容页 + 顶部三个 Tab（站内信、短信、邮件）。站内信列表字段为 ID、分类、标题、全部通知、已发送、排序，操作为详情、删除、接收情况；短信列表字段为业务名称、业务编码、平台、启动状态，操作为测试、修改、删除、日志，日志查看发送历史；邮件列表字段为业务名称、业务编码、服务地址、启动状态，操作为测试、修改、删除、日志，日志查看发送历史。
