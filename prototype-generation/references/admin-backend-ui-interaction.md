...
- 顶栏右侧消息提醒使用铃铛 + 未读数；点击铃铛先展示小下拉窗口，下拉窗口展示最近提醒，右上角必须有 `全部` 链接；点击 `全部` 后打开右侧抽屉展示完整消息列表。
- 顶栏右侧必须包含当前登录账号头像和名称；点击后下拉展示 `个人中心`、`退出登录`。
- `个人中心` 使用右侧抽屉，不作为左侧菜单项；抽屉包含 `基本信息`、`安全信息` 两个 Tab。基本信息展示用户名、姓名/显示名、手机号、邮箱、所属部门、岗位等，支持更新个人信息；安全信息支持修改登录密码，包含原密码、新密码、确认密码和保存反馈。
...
- PC 管理后台默认侧边栏顺序为 `首页`、`业务平台`、`统计平台`、`系统管理`，除非用户明确排除某一类。
- `首页` 是第一个顶层入口和默认进入页，定位为当前登录用户个人工作台，不要做成营销式 Hero、空欢迎页，或直接复用统计平台的运营驾驶舱。
- 首页工作台应展示与当前用户相关的待办任务、通知提醒、抄送消息、待审批/待签收事项、近期关注项目或业务对象、个人关键指标和快捷入口；内容必须按角色权限、按钮权限、数据范围和消息已读/未读状态过滤。
- 首页中的待办、通知和快捷入口必须能跳转到对应列表、详情、弹窗或抽屉，例如待办任务、全部消息抽屉、项目台账、患者线索、绩效管理、统计分析、消息推送等。
- 业务处理功能统一聚合到 `业务平台` 顶层菜单下，可按业务台账、协同办公、协同与绩效等二级菜单组织。
- 统计类功能统一聚合到 `统计平台` 顶层菜单下，包括运营驾驶舱、统计分析、报表中心、指标看板、BI 分析、趋势分析、汇总查询、导出中心、HR/财务导出等看板/统计/报表/导出能力。
- 系统基础能力统一聚合到 `系统管理` 顶层菜单下，点击系统管理后展示 `权限管理`、`日志管理`、`令牌管理`、`敏感词管理`、`参数管理`、`消息推送`。
- `权限管理` 和 `日志管理` 是系统管理下的二级主菜单，必须支持展开/收回；权限管理下包含用户管理、角色管理、菜单管理、部门管理、岗位管理；日志管理下包含审计日志、操作日志。
- `令牌管理`、`敏感词管理`、`参数管理`、`消息推送` 作为系统管理下的直接页面入口，不要与业务台账、协同办公等业务菜单混放。
- 展开态要有明确的箭头或图标状态，当前激活页所在的父级菜单应保持高亮或弱高亮。
...
6. 数据表格：带 loading、border、居中表头/内容、溢出 tooltip；PC 管理后台列表页最左侧默认提供复选框选择列，用于后续批量操作。
7. 分页：右下，顶部 16px 间距。
...
- `el-table border`。
- loading 状态覆盖表格。
- 选择列宽 40px，PC 管理后台列表页最左侧必须是复选框选择列，表头提供全选复选框，行内提供单行选择复选框。
- 复选框选择列不等于必须展示批量按钮；没有被需求确认的批量业务动作时，只保留选择列，不额外生成“选中项处理”按钮。
- 序号列宽 60px，常固定左侧。
- 关键标识列可固定左侧。
...
- 必须使用全宽标准 CRUD 列表页，不得使用“左侧角色列表 + 右侧权限明细/权限摘要”的左右分栏。
- 列表列可包含角色编码、角色名称、菜单范围、数据范围、按钮权限、状态；权限摘要只在表格列中简要展示。
- 行内操作使用编辑、权限配置、启用/停用等按钮。
- 菜单权限树、按钮权限、数据范围配置放在权限配置弹窗或抽屉中完成，保存后回到列表刷新。
- 顶部可保留新增角色、权限配置模板等操作按钮，但不要在页面右侧常驻权限树。

### 6.2 左树右表页

仅用于组织、部门、区域、层级主数据、树形字典等真实层级结构：
...
用于不同业务分类及各分类明细表格的管理页：

- 不得使用左侧分类导航 + 右侧明细表格的左右分栏。
- 使用全宽内容页，顶部放分类 Tab 标签；如果业务存在三个分类，必须使用三个顶部 Tab。
- 每个 Tab 内部保留完整的查询区、操作栏、数据表格、分页、行内操作、弹窗/抽屉和状态反馈。
- Tab 切换后只刷新当前分类的数据和筛选条件，不改变侧边主菜单和 TagsView。
- 消息推送等站内信/短信/邮件类页面必须使用该模式，并默认位于系统管理下。
...
- 表单项底部间距常用 20px。
- 弹窗内两列表单、提示说明、权限树、表格和 textarea 等直接相邻内容块之间必须保留 16-20px 垂直间距；textarea 不得贴紧上一行输入框、下拉框或说明文字。
- 当两列表单后接“审批意见、补充说明、变更原因、测试内容、删除原因”等 textarea 时，textarea 应单独占满整行，并与上方表单保持至少 16px 间距。
...
- Body 内直接相邻内容块使用统一垂直节奏，默认 16px；表单网格后接 textarea 时建议 18-20px，避免多行输入区域与上方控件黏连。
...
- 首页工作台：位于侧边栏第一项，展示当前登录用户相关的快捷入口、待办任务、通知提醒、抄送消息、近期关注、个人指标、日志或公告；不得只展示静态欢迎语。
- 标准列表页：搜索、操作栏、表格、分页、右上工具栏。
- 左树右表页：组织、部门、区域或层级字典管理。
- 新增编辑弹窗：两列表单、校验、loading、确认取消。
- 弹窗多行输入态：textarea 与上方组件保持独立间距，重点检查审批意见、补充说明、变更原因、驳回原因、测试内容、配置说明等场景。
- 详情页或抽屉：只读信息、流程记录、操作日志。
- 字典/状态管理页：树节点 hover 操作、右侧明细。
- 权限敏感页：按钮权限隐藏、禁用 tooltip、不可选择行。
- 顶栏通知菜单：右上角铃铛 + 未读数；点击后出现小下拉窗口，下拉右上角有“全部”；点击全部打开右侧抽屉完整消息列表，支持全部已读、单条已读、查看来源。
- 顶栏账号菜单：右上角头像 + 名称 + 下拉；下拉包含个人中心、退出登录；个人中心为右侧抽屉，含基本信息、安全信息两个 Tab，支持更新个人信息和修改登录密码。
- 角色管理页：全宽标准列表 + 权限配置弹窗/抽屉，不得使用左侧角色列表 + 右侧权限摘要的左右分栏。
- 令牌管理页：全宽标准列表，字段为用户名、客户端、令牌、过期时间、状态，行内操作为下线，下线必须弹窗填写原因并记录操作留痕；默认不展示“下线选中令牌”或“批量下线”按钮，除非用户明确要求批量下线。
- 参数管理页：全宽标准列表，覆盖系统规则和业务规则，字段为名称、键、值、状态、系统标识，行内操作为修改，修改必须记录原因和审计日志；默认不展示“修改选中参数”按钮。
- 敏感词管理页：全宽标准列表，字段为敏感词、类型、备注，类型只能是白名单或黑名单，操作列保留修改、删除等行内操作；除非用户明确要求高级规则治理，不默认展示规则名称、适用范围、处理策略、状态等列。
- 分类明细页：不同业务分类及其明细表格不得左右分栏；使用全宽内容页 + 顶部 Tab 分类标签，三个分类时使用三个 Tab。
- 消息推送页：位于系统管理下，不得使用左右分栏；使用全宽内容页 + 顶部三个 Tab（站内信、短信、邮件）。站内信列表字段为 ID、分类、标题、全部通知、已发送、排序，操作为详情、删除、接收情况；短信列表字段为业务名称、业务编码、平台、启动状态，操作为测试、修改、删除、日志，日志查看发送历史；邮件列表字段为业务名称、业务编码、服务地址、启动状态，操作为测试、修改、删除、日志，日志查看发送历史。
