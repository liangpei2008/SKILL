...
**涉及客户端/访问端**：列出本流程涉及的 PC 管理后台、微信小程序、移动 H5、企业微信/钉钉工作台、独立 App、患者端、外部服务商端或其他访问端；如某端明确不涉及，应在总体范围或附录B中记录为已排除或待确认

**流程步骤**（按顺序列出，每一步标注步骤类型）：

| 步骤 | 步骤描述 | 发生端/访问端 | 引用的业务功能 | 衔接类型 | 分支条件 |
|------|---------|--------------|----------------|---------|---------|
| 1 | 录入合同 | PC 管理后台 | FUNC-CON-001 | 同步等待 | - |
| 2 | 系统自动生成付款条款 | 系统后台 | FUNC-PMT-001 | 异步触发 | - |
| 3 | 录入开票信息（可重复执行） | PC 管理后台 | FUNC-INV-001 | 同步等待 | - |
| 4 | 录入收款信息 | PC 管理后台 | FUNC-INV-002 | 同步等待 | - |
| 5 | 判断是否全款回收 | 系统后台 | - | 条件分支 | 是→步骤6；否→返回步骤3 |
| 6 | 关闭合同 | PC 管理后台 | FUNC-CON-002 | 同步等待 | - |
...
- 操作完成后是否会自动触发别的事情？
- 操作完成后是否需要产生协同办公动作或信息推送？
- 如果用户可自主上传、发布、评论、留言或编辑富文本/附件说明，是否需要敏感词管理或内容审核？
- 这个功能涉及哪些独立的业务规则？
...
**适用客户端/访问端**：PC 管理后台（如同时支持微信小程序、移动 H5、企业微信/钉钉工作台、独立 App、患者端或外部服务商端，需逐一说明；不适用时写 `不适用`）
...
**协同办公与信息推送**：
- 协同办公：是否产生待办、评论、附件流转、转办或交接事项；不适用时填写 `不适用`
- 信息推送：是否通过站内信、短信、邮件、企业微信/钉钉等渠道通知相关角色；需说明触发条件、接收角色、渠道、模板/内容、频率/时点、失败处理和留痕要求

**敏感词与内容审核**：
- 适用条件：当用户可自主上传、发布、评论、留言或编辑富文本/附件说明时必须确认是否适用
- 说明要求：覆盖内容范围、敏感词库维护角色、命中处理方式、人工审核、复核/申诉、留痕要求和验收口径；不适用时填写 `不适用`
...
| 适用客户端/访问端 | 无法判断该功能在哪些端可用，容易导致原型、权限、接口和验收范围遗漏 |
...
| 协同办公与信息推送 | 无法判断是否需要待办、协同处理、站内信、短信、邮件等跨角色提醒与通知能力 |
| 敏感词与内容审核 | 用户可自主产生内容时，无法判断是否需要内容安全控制、人工审核、复核和留痕 |
...
**协同办公与信息推送**用于确认流程是否需要跨角色协作和主动提醒。只要业务动作可能要求他人处理、知会、补充材料、确认结果或及时响应，就必须询问是否需要待办、评论、附件流转、站内信、短信、邮件、企业微信/钉钉等能力；不需要时显式填写 `不适用`。

**敏感词与内容审核**用于确认用户自主内容是否需要内容安全控制。只要业务允许用户上传、发布、评论、留言、编辑富文本或提交附件说明，就必须询问是否需要敏感词库、命中处理、人工审核、复核/申诉和留痕；不需要时显式填写 `不适用`。
...
**客户端/访问端不是 UI 设计细节，而是系统边界、角色权限和交付范围的一部分。** 在需求探索阶段必须明确哪些端纳入一期，哪些端排除或待确认。
...
每个端至少需要说明：使用角色、主要业务用途、是否纳入一期、是否允许录入/审批/导出/仅查看、是否涉及外部用户登录，以及与角色数据权限是否一致。不得把“有无患者端、外部服务商端、移动端、小程序端”留到原型或架构阶段才首次确认。
...
- [ ] 是否已明确系统涉及的客户端/访问端，且每个核心流程步骤标注了发生端或访问端
...
- [ ] 每个业务功能是否都明确了适用客户端/访问端
- [ ] 每个业务功能是否都说明了前置条件
- [ ] 每个业务功能是否都明确区分了"后置状态变更"和"下游触发"两个独立字段
- [ ] 每个核心业务功能是否确认了协同办公与信息推送是否适用；适用时是否说明触发场景、接收角色、渠道、模板/内容、频率/时点、失败处理和留痕要求
- [ ] 对用户可自主上传、发布、评论、留言或编辑富文本/附件说明的功能，是否确认了敏感词管理或内容审核是否适用；适用时是否说明内容范围、词库维护、命中处理、人工审核、复核/申诉、留痕和验收口径
...
**岗位角色（对应 M5）**
- [ ] 是否列出了业务涉及的全部岗位角色
- [ ] 是否列出了各角色可使用的客户端/访问端，并说明每个端的主要用途
- [ ] 每个角色的功能权限是否完整列出
...
*文档版本 v1.0.1 | 软件需求文档编写规范 | 2026年7月*
