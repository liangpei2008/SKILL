...
9. 系统一期需要建设哪些客户端/访问端，以及每个端服务的角色和能力边界；例如 PC 管理后台、微信小程序、移动 H5、企业微信/钉钉工作台、独立 App、患者端、外部服务商端、第三方系统入口等
...
4. 列出你初步识别或需要确认的客户端/访问端清单，明确哪些端可能纳入一期、哪些端排除或待确认
5. 请求用户确认这个总体范围是否正确，是否有遗漏的业务模块
...
3. 对每个核心流程确认流程步骤发生在哪些客户端/访问端，哪些端只查看，哪些端允许录入、审批、导出、消息提醒或轻量处理
4. **逐一列出文档中出现的核心业务单据**，针对每一个单据明确发起审批流确认提问（这是强制项，不可省略或合并简化）
5. 如用户确认存在审批流，进一步收集审批节点、审批角色、通过/驳回后状态流转，并生成对应的 Mermaid 审批流程图
...
4. **针对每一个查询/列表类业务功能，逐一向用户确认各角色的数据范围**，这是强制项
5. 对撤回、作废、更正、重开等敏感逆向操作，逐一确认可执行或审批的角色
...
*文档版本 v1.0.1 | AI 需求探索与确认提示词 | 2026年7月*
